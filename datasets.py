...
from prepossessPipeline import prepossessPipeline
...
class KaggleEDFMASD():
    ...
    def prepossessing(self, n_subjects, inp_dir='../../EEG Data/', windows=2):
        fs = 128
        mkpt1 = int(fs * 10 * 60)
        mkpt2 = int(fs * 20 * 60)
        ...
        for s in range(1, n_subjects + 1):
            ...
            for i, t in enumerate(subject_map[s]):
                ...
                eeg = prepossessPipeline(data=eeg, fs=fs).bandPassFilter(fs_highpass=0.5, fs_lowpass=40, module='scipy.signal')

                # (7, timeSeries) -> (n, 7, 128*windows)
                eeg = eeg.reshape((7, -1, 128 * windows), order='C').transpose(1, 0, 2)
            ...
            with open(f'../../EEG Data/Disposed/withoutFeatureExtration/subject_{s}.pkl', 'wb') as f:
                pickle.dump(data, f, pickle.HIGHEST_PROTOCOL)

class data_20211114_ZhangHao:
    def __init__(self):
        None
    def loadData(self, classification=2, format='3D'):
        '''
        :param classification: obtain 2 classification data or 4 classification data
        :param format:
            '3D' -- X with shape (n, H, W, C) -- y in one-hot format --  for CNN input;
            '2D' -- X with shape  (n, time_step, vector_num) -- y in one-hot format -- for RNN input;
            '1D' -- X with shape (n, m) -- y -- for SVM input;
        ...
        '''
        if classification == 2:
            data = loadmat('../../EEG Data/data_20211114/2class.mat')
        elif classification == 4:
            data = loadmat('../../EEG Data/data_20211114/4class.mat')
        else:
            print('Invalid Param: "classification", which shoule be choosed in [2, 4]')
            return None
        ...
        Xtrain = prepossessPipeline(data=Xtrain, fs=None).segment_dataset(400, 50) # (4608,15,400,22)
        Xtest = prepossessPipeline(data=Xtest, fs=None).segment_dataset(400, 50)  # （576,15,400,22）
        ...
        ytrain = tf.keras.utils.to_categorical(ytrain, classification).astype('int8')
        ytest = tf.keras.utils.to_categorical(ytest, classification).astype('int8')
        return Xtrain, Xtest, ytrain, ytest
...
if __name__ == '__main__':
    ...
    Xtrain, Xtest, ytrain, ytest = data_20211114_ZhangHao().loadData(classification=2)
    print(Xtrain.shape)
    print(ytrain.shape)
    print(ytest.shape)
    print(Xtest.shape)
